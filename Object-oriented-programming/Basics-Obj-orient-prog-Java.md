...
### Переопределение методов в базовом классе

Если в классе Employee и в подклассе Doctor есть метод с одинаковым именем и если тип переменной, в к-ой хранится ссылка на объект подкласса Doctor, является классом Employee (как в примере выше), то для данного метода будет вызвана реализация базового класса Employee, а не реализация в подкласса Doctor. 

Чтобы в этом случае вызывалась именно реализация подкласса Doctor, данный метод в подклассе Doctor необходимо *переопределить* (override) и в базовом класса Employee сделать его **виртуальным**.   
...
### Как работает переопределение методов в базовом классе
