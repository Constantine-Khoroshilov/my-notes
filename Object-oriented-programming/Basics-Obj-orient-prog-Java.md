...
1. В родительском (Employee) и дочернем (Doctor) классе есть метод с одинаковым именем (например, getSalary)
...
Для данного метода будет вызвана реализация родительского класса (Employee), а не реализация дочернего (Doctor). 
